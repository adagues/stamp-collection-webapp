...
export default function SearchWorkbench() {
  ...
  const [progress, setProgress] = useState<IndexProgress | null>(null), [counts, setCounts] = useState({ visual: 0, semantic: 0, total: 0 });
  ...
  async function refreshCounts() {
    try {
      ...
      setCounts({ visual: visual.ready.length, semantic: semantic.ready.length, total: visual.ready.length + visual.pending.length });
  ...
  async function prepare(kind: EmbeddingKind) {
    ...
    try {
      ...
      await prepareCatalog(kind, value => { setProgress(value); setMessage(`Préparation : ${value.done} / ${value.total}. ${value.failed ? `${value.failed} illustration(s) ou notice(s) non traitée(s).` : ''}`); }, controller.current.signal);
  ...
  async function search(kind?: EmbeddingKind) {
    ...
    try {
      ...
      if (kind) {
        if (counts[kind] === 0) throw new Error('Préparez d’abord le catalogue avec le bouton correspondant ci-dessous.');
        try {
          ...
          if (kind === 'visual') throw new Error('Le modèle visuel est indisponible. Réessayez ou parcourez le catalogue manuellement.');
