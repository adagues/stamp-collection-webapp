...
async function main() {
  ...
  if (!rows.length || fields.some(key => !(key in rows[0]))) throw new ImportError('Le fichier doit contenir toutes les colonnes du schéma.');
  ...
  const stamps = rows.map((row, index): Stamp => {
    ...
    if (!/^[a-zA-Z0-9_-]{1,100}$/.test(row.id) || ids.has(row.id) || !Number.isInteger(year) || year < 1840 || year > 2100 ||
      ...
      throw new ImportError(`Ligne ${index+2} : données invalides ou identifiant dupliqué.`);
    ids.add(row.id);
