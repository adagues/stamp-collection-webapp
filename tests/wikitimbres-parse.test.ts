...
describe('extraction locale Wikitimbres', () => {
  it('extrait les faits, décode les entités et exclut illustrations et prose', () => {
    ...
    expect(row).toEqual({
      ...
      series: 'Paysages, "grand format"', denomination: '15 F',
      description: 'Pays : France ; Année : 1950 ; Valeur faciale : 15 F ; Couleur : bleu & vert ; Thème : Paysages',
  ...
  it('ignore les pages incomplètes, les années invalides et les titres trop longs', () => {
    ...
    expect(parseStamp(fixture.replace('France</td>', '</td>'), 1, source)).toBeNull();
