...
describe('Routes API', () => {
  ...
  it('enregistre la collection par la route publique et refuse les identifiants absents', async () => {
    const response = await PUT(request('collection',{ owned:true, quantity:4, personal_reference:'Album bleu' }), { params:{ id:'demo-0001' } });
  ...
  it('rejette une quantité incorrecte avec un message français', async () => {
    const response = await PUT(request('collection',{ owned:true, quantity:-2, personal_reference:'' }), { params:{ id:'demo-0001' } });
  ...
  it('recherche avec les accents et renvoie l’état de possession', async () => {
    const response = await search(request('search',{ text:'Éclat' }));
