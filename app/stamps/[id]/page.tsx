...
export async function generateMetadata({ params }: { params: { id: string } }) {
  return { title: `${getStamp(params.id)?.title || 'Timbre introuvable'} — Coffre à Timbres` };
}
export default function StampDetail({ params }: { params: { id: string } }) {
  const stamp = getStamp(params.id); if (!stamp) notFound();
